...
def load_mnist_data(img_rows, img_cols):
    ...
    X_train = np.array([cv2.resize(X_train[img], (img_rows,img_cols)) for img in range(len(X_train))])
    X_valid = np.array([cv2.resize(X_valid[img], (img_rows,img_cols)) for img in range(len(X_valid))])
...
X_train, Y_train, X_test, Y_test = load_mnist_data(124,124)
...
def inception_module(x,
    ...
    pool_proj = Conv2D(filters_pool_proj, (1,1), padding='same')(x)
...
x = MaxPooling2D((3,3), padding='same', strides=(2,2),
